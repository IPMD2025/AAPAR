...
def main(cfg, args):
    ...
    print(valid_tsfm)
    save_path=osp.join(args.test_img, 'Pad_datasets1')
    
    # may_mkdirs(save_path+'/rgb')
    ...
    with torch.no_grad():
        for name in os.listdir(args.test_img+'/rgb'):
            if name == 'train' or name == 'val':
                ...
                pdirs.sort()
                # may_mkdirs(save_path+'/rgb'+"/"+name)
                for pdir in pdirs:
                    may_mkdirs(pdir)
                    pid = int(osp.basename(pdir))
                    img_dirs = glob.glob(osp.join(pdir, '*.jpg'))
                    may_mkdirs(save_path+"/rgb/"+name+"/"+osp.basename(pdir))
                    for img_dir in img_dirs:
                        image_save_path=save_path+"/rgb/"+name+"/"+osp.basename(pdir)+"/"+osp.basename(img_dir)
                        # if os.path.exists(image_save_path):
                        #         continue
                        # else:
                        #     img=cv2.imread(img_dir)#112*410,#1108
                        ...
                        for i, val in enumerate(valid_probs):
                            if val:
                                with open(args.test_img + '/' + 'PAR_PETA_1220_0.5.txt', 'a',
                                          encoding='utf-8') as f:
                ...
                for cam in ['A', 'B', 'C']:
                    ...
                    for pdir in pdirs:
                        pid = int(osp.basename(pdir))
                        img_dirs = glob.glob(osp.join(pdir, '*.jpg'))
                        may_mkdirs(save_path+"/rgb/test/"+cam+"/"+osp.basename(pdir))
                        for img_dir in img_dirs:
                            image_save_path=save_path+"/rgb/"+name+"/"+cam+"/"+osp.basename(pdir)+"/"+osp.basename(img_dir)
                            # if os.path.exists(image_save_path):
                            #         continue
                            # else:
                            #     img=cv2.imread(img_dir)#112*410,#1108
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        default="/media/data2/lx/cloth-changing/dataset/prcc",
    )
    parser.add_argument(
