...
def main(cfg, args):
    ...
    with torch.no_grad():
        for file_name in os.listdir(args.test_img):
            if file_name != 'info' and file_name !='Pad_datasets':
                img_paths = glob.glob(osp.join(args.test_img+'/'+file_name, '*.png'))
                ...
                for img_path in img_paths:
                    ...
                    for i, val in enumerate(valid_probs):
                        if val:
                            with open(args.test_img + '/' + 'PAR_PETA_SEM_1220_0.5.txt', 'a',
                                        encoding='utf-8') as f:
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        default="/media/data2/lx/cloth-changing/dataset/LTCC_ReID",
    )
    parser.add_argument(
