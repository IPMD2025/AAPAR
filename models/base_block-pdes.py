...
class TransformerClassifier(nn.Module):
    def __init__(self,args, attr_num,attr_words,des_num,des_num_u,des_num_l,des_num_b,des_num_g, dim=768, pretrain_path='/media/backup/**/pretrained/jx_vit_base_p16_224-80ecf9dd.pth'):#
        super().__init__()
        ...
        self.bn_des_u = nn.BatchNorm1d(des_num_u)
        self.bn_des_l = nn.BatchNorm1d(des_num_l)
        self.bn_des_b = nn.BatchNorm1d(des_num_b)
        self.bn_des_g = nn.BatchNorm1d(des_num_g)
    ...
    def forward(self, imgs,ViT_model,des=None,des_u=None,des_l=None,des_b=None,des_g=None,gt_label=None):#,group_arr
        ...
        if des!=None:
            ...
            loss_u,_ = ViT_model.forward_aggregate(features[:,1], des_u)
            loss_u = self.bn_des_u(loss_u)
            loss_l,_ = ViT_model.forward_aggregate(features[:,2], des_l)
            loss_l = self.bn_des_l(loss_l)
            loss_b,_ = ViT_model.forward_aggregate(features[:,3], des_b)
            loss_b = self.bn_des_b(loss_b)
            loss_g,_ = ViT_model.forward_aggregate(features[:,4], des_g)
            loss_g = self.bn_des_g(loss_g)
        ...
        feat_g=self.head(img_ft[:,0])#img_ft[:,0]
       ...
       ###########################
        ...
        for i in range(self.lmbd):
            att_vit_temp=torch.cat((att_vit[:,i+1].unsqueeze(1),att_vit[:,1+self.lmbd:1+self.lmbd + P],att_vit[:,1+self.lmbd + img_msk_start[i] : 1+self.lmbd + img_msk_end[i]]),dim=1)#
            att_vit_p=torch.cat((att_vit_temp[:,:,i+1].unsqueeze(-1),att_vit_temp[:,:,1+self.lmbd:1+self.lmbd + P],att_vit_temp[:,:,1+self.lmbd + img_msk_start[i] : 1+self.lmbd + img_msk_end[i]]),dim=-1)#
            last_map = att_vit_p[:,0,1:].unsqueeze(1)
            img_patch= torch.cat((img_ft[:,1+self.lmbd:1+self.lmbd + P],img_ft[:,1+self.lmbd + img_msk_start[i] : 1+self.lmbd + img_msk_end[i]]),dim=1)#
        ...
        tex_embed =word_embed#torch.gather(word_embed,dim=1,index=torch.tensor(self.group_vice).unsqueeze(0).unsqueeze(-1).expand(B,-1,self.dim).cuda())## + self.tex_embed
        vis_embed = feat_cam#img_ft#torch.cat(([:,:1+self.lmbd],),dim=1)#img_ft[:,1+self.lmbd:]#global_cam[:,0].unsqueeze(1)#+ self.pos_embed #+ self.vis_embed #features_cls_temp #features[:,5:]
        ...
        if des!=None:
            ...
            logits.append(loss_u)
            logits.append(loss_l)
            logits.append(loss_b)
            logits.append(loss_g)
    ...
    def get_image_mask(self,N,C):
        ...
        # self.image_mask[4][C+P:].fill_(float("-inf"))
        for i in range(C): 
            if i!=0 :#and i!=4
                self.image_mask[i][C:P+C].fill_(float("-inf"))#
                # self.image_mask[i,C+i*P//5:C+(i+1)*P//5].fill_(0)
    ...
    #     self.image_mask[:C,:C].fill_(float("-inf"))     #8个cls token   
    #     self.image_mask[1][C+P+5*N//8:].fill_(float("-inf"))   #0-hair， 1th，2th，3th块保留  1-age whole attention 2-gender whole attention
    #     self.image_mask[2][:C+P+3*N//8].fill_(float("-inf"))
    #     self.image_mask[3][:C+P+2*N//8].fill_(float("-inf"))   #3-carry 3,4,5,6块保留 [2*2*14+8,6*2*14+8]
    #     self.image_mask[3][C+P+N*6//8:].fill_(float("-inf"))  #4-accessory 1,2,3,4,5,6块保留 [6*2*14+8]  
