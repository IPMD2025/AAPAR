...
class TransformerClassifier(nn.Module):
    def __init__(self,args, attr_num,attr_words,des_num, dim=768, pretrain_path='/media/backup/**/pretrained/jx_vit_base_p16_224-80ecf9dd.pth'):#
        super().__init__()
        ...
        for i in range(self.lmbd):
            vit_p = vit_base()
            vit_p.load_param(pretrain_path)
            self.blocks_p.append(vit_p.blocks[-1:])
            self.norm_p.append(vit_p.norm)
        ...
        # z = 1 - x - y
        # samples = nn.Parameter(torch.tensor([x,y,z])).cuda()
        ...
        # d = 1 - a - b - c
        # samples_l = nn.Parameter(torch.tensor([a,b,c,d])).cuda()
    ...
    def forward(self, imgs,ViT_model,des=None,gt_label=None):#,group_arr
        ...
        part_tokens= x[:,0].unsqueeze(1) + self.cls_part_token
       ...
       ###########################
        ...
        feat_map_g=F.relu(img_ft[:,1:])#+self.lmbd 
        feat_cam_g=last_map@feat_map_g


        img_msk_start=[P,P+self.patch // 2,P+self.patch // 4,0]
        img_msk_end=[P+self.patch // 2,P+self.patch +1,P+self.patch * 3 // 4,P]
        # img_msk_start=[0,3*self.patch //8,2*self.patch // 8,0]
        # img_msk_end=[5*self.patch // 8,self.patch +1,6*self.patch // 8,self.patch +1]
        ...
        for i in range(self.lmbd):
            ...
            self.norm_p[i].cuda()
            x = torch.cat((part_tokens[:,i].unsqueeze(1),features[:,img_msk_start[i]:img_msk_end[i]]),dim=1)
            for blk in self.blocks_p[i]:
                x = blk(x)
            img_ft_p.append(self.norm_p[i](x))
            self.head_p[i].cuda()
            feat_p=self.head_p[i](img_ft_p[i][:,0])
        ...
        for i in range(self.lmbd):
            att_vit_p=self.blocks_p[i][0].attn_w.float()
            last_map = att_vit_p[:,0,1:].unsqueeze(1)
            img_patch= img_ft_p[i][:,1:]#torch.cat((img_ft[:,1+self.lmbd:1+self.lmbd + P],),dim=1)#
