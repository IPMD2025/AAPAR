...
class TransformerClassifier(nn.Module):
    def __init__(self,args, attr_num,attr_words,des_num, dim=768, pretrain_path='/media/backup/**/pretrained/jx_vit_base_p16_224-80ecf9dd.pth'):#
        ...
        # self.head_des = nn.Linear(dim, des_num)
    ...
    def forward(self, imgs,ViT_model,des=None,gt_label=None):#,group_arr
        ...
        word_embed_temp=ViT_model.encode_text(self.text,attr=True).cuda().float()
        
        # logits_des=self.bn_des(self.head_des(features[:,0]))
        # destoken = clip.tokenize(des,truncate=True).cuda()
        # des_embed = ViT_model.encode_text(destoken).cuda().float()
        if des!=None:
            # loss_itc = get_contrastive_loss(features[:,0], des,ViT_model)
            loss_itc,_ = ViT_model.forward_aggregate(features[:,0], des)
            loss_itc = self.bn_des(loss_itc)
        # final_similarity,logits_per_image = ViT_model.forward_aggregate(features[:,0],word_embed_temp)#
        ...
        # att_vit_cls=torch.cat((att_vit[:,0].unsqueeze(1),att_vit[:,1+self.lmbd+ P:]),dim=1)# 
        # att_vit_cls=torch.cat((att_vit_cls[:,:,0].unsqueeze(-1),att_vit_cls[:,:,1+self.lmbd+ P :]),dim=-1)#+ P
        # last_map = att_vit_cls[:,0,1:].unsqueeze(1)
        last_map = att_vit[:,0,1+self.lmbd:].unsqueeze(1)
        feat_map_g=F.relu(img_ft[:,1+self.lmbd:])#+self.lmbd+ P 
        ...
        for i in range(self.lmbd):
            att_vit_temp=torch.cat((att_vit[:,i+1].unsqueeze(1),att_vit[:,1+self.lmbd:1+self.lmbd + P],att_vit[:,1+self.lmbd + P + img_msk_start[i] : 1+self.lmbd + P + img_msk_end[i]]),dim=1)#
            att_vit_p=torch.cat((att_vit_temp[:,:,i+1].unsqueeze(-1),att_vit_temp[:,:,1+self.lmbd:1+self.lmbd + P],att_vit_temp[:,:,1+self.lmbd +P+ img_msk_start[i] : 1+self.lmbd + P + img_msk_end[i]]),dim=-1)#
            last_map = att_vit_p[:,0,1:].unsqueeze(1)
            img_patch= torch.cat((img_ft[:,1+self.lmbd:1+self.lmbd + P],img_ft[:,1+self.lmbd + P + img_msk_start[i] : 1+self.lmbd + P + img_msk_end[i]]),dim=1)#
        ...
        vis_embed = feat_cam#img_ft#[:,1+self.lmbd:]#global_cam[:,0].unsqueeze(1)#+ self.pos_embed #+ self.vis_embed #features_cls_temp #features[:,5:]
        ...
        # pp=torch.gather(d,dim=1,index=torch.tensor(group_order).unsqueeze(0).expand(B,-1).cuda())#self.group_viceself.get_groupvice(group_ind)
        # logits.append(self.bn(pp))#
        logits.append(self.bn(d))
        # logits.append(final_similarity)
        if des!=None:
