...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, imgs,ViT_model,des=None,gt_label=None):#,group_arr
       ...
       ###########################
        ...
        #     feat_map=F.relu(img_ft[:,1+self.lmbd:])#+self.lmbd 
        ...
        vis_embed = img_ft#feat_cam#torch.cat((img_ft[:,:1+self.lmbd],),dim=1)#img_ft[:,1+self.lmbd:]#global_cam[:,0].unsqueeze(1)#+ self.pos_embed #+ self.vis_embed #features_cls_temp #features[:,5:]
