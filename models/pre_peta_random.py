...
class petabaseDataset(Dataset):
    def __init__(self, root_path):
       ...
       'Sunglasses','Trousers', 'Tshirt','Other', 'VNeck',]
        ...
        self.raw_words=['personalLess30', 'personalLess45', 'personalLess60', 'personalLarger60', 'carryingBackpack', 
        'carryingOther', 'lowerBodyCasual', 'upperBodyCasual', 'lowerBodyFormal', 'upperBodyFormal', 
        'accessoryHat', 'upperBodyJacket', 'lowerBodyJeans', 'footwearLeatherShoes', 'upperBodyLogo', 
        'hairLong', 'personalMale', 'carryingMessengerBag', 'accessoryMuffler', 'accessoryNothing', 
        'carryingNothing', 'upperBodyPlaid', 'carryingPlasticBags', 'footwearSandals', 'footwearShoes', 
    ...
    def get_one_target(self,caption):
        ...
        target["hair"]=hair if hair!=[] else ['unknown']
        target["age"]=age if age!=[] else ['unknown']  
        target["gender"]=gender if gender!=[] else ['unknown']

        target["carry"]=carry if carry!=[] else ['unknown']
        target["accessory"]=accessory if accessory!=[] else ['unknown']

        target["upperbody"]=upper if upper!=[] else ['unknown']
        target["lowerbody"]=lower if lower!=[] else ['unknown']   
        target["foot"]=foot if foot!=[] else ['unknown']
    ...
    def get_caption(self,label=None):
        group_target=[]
        description=[]
        for i in range(label.shape[0]):
            ...
            caption=[]
            for ite in index[0]:                       
                at=self.raw_words[ite]
                caption.append(at)
            target, captions = self.get_one_target(caption) 
            group_target.append(target)
            description.append(captions)
        return group_target,description
            ...
            if phase=="test":
                if key=="test":
                    for i in item:
                        ...
                        imagename=os.path.join(root_path,"images", dataset['image'][i])      #"images",           
...
if __name__ == '__main__':
    ...
    root_path="/media/backup/**/PETA/"#"/raid2/yue/datasets/Attribute-Recognition/PETA/PETA_select/PETAdata/"
    #save_root="../../dataset/PETA_select_35/"
    save_root="/media/backup/**/PETA/PETA_train_label/" #"../../dataset/PETAdata/PETA_train_label/"
    ...
    label=np.ones((3,35))
    group_target,description=petadata.get_caption(label)
