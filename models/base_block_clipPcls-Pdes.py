...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, imgs,ViT_model,des=None,des_u=None,des_l=None,des_b=None,des_g=None,gt_label=None):#,group_arr
       ...
       ###########################
        ...
        feat_cam_tmp=[]
        for i in range(1+self.lmbd):
            last_map = att_vit[:,i,1+self.lmbd:].unsqueeze(1)
        ...
        # logits_agg = self.samples[0]*logits[0] + self.samples[1]*logits[1] + self.samples[2]*logits[2]
        # logits.append(logits_agg)
        # logits_agg = self.bn_agg(self.head_agg(self.aggregate(torch.cat((img_ft[:,:1+self.lmbd],ptext_ft[:,:self.attr_num]),dim=1)).squeeze(1)))
        # logits.append(logits_agg)
        if des!=None:
    ...
    def get_image_mask(self,N,C):
        ...
        for i in range(C): 
            # if i!=0 :#and i!=4
            self.image_mask[i][C:P+C].fill_(0)#float("-inf")
