...
def main(cfg, args):
    ...
    with torch.no_grad():
        for childdir in os.listdir(args.test_img):
            if childdir !='Pad_datasets' and childdir=='train':
                # continue
                may_mkdirs(save_path +"/"+childdir)
                temp_dir=args.test_img+"/"+childdir
                for file_name in os.listdir(temp_dir):
                    img_paths = glob.glob(osp.join(args.test_img+'/'+childdir+'/'+file_name, '*.jpg'))
                    may_mkdirs(save_path+"/"+childdir+"/"+file_name)
                    for img_path in img_paths:
                        imagename,_ =osp.splitext(osp.basename(img_path))
                        # image_save_path=save_path+"/sem"+"/"+file_name+"/"+imagename+".jpg"
                        image_save_path=save_path+"/"+childdir+"/"+file_name+"/"+imagename+".jpg"
                        ...
                        for i, val in enumerate(valid_probs):
                            if val:
                                with open(args.test_img + '/' + 'PAR_PETA_0224_0.5.txt', 'a',
                                            encoding='utf-8') as f:
                                    f.write(image_save_path+' '+ str(i)+' '+ str(1) + '\n')#img_dir
                            else:
                                with open(args.test_img + '/' + 'PAR_PETA_0224_0.5.txt', 'a',
                                            encoding='utf-8') as f:
                                    f.write(image_save_path+' '+ str(i)+' '+ str(0) + '\n')#img_dir
            elif childdir=='val' or childdir=='test':
                may_mkdirs(save_path +"/"+childdir)
                temp_dir=args.test_img+"/"+childdir
                for schilddir in os.listdir(temp_dir):
                    may_mkdirs(save_path +"/"+childdir+"/"+schilddir)
                    if schilddir=='query':
                        # continue
                        img_paths = glob.glob(osp.join(args.test_img+'/'+childdir+'/'+schilddir, '*.jpg'))
                        for img_path in img_paths:
                            imagename,_ =osp.splitext(osp.basename(img_path))
                            # image_save_path=save_path+"/sem"+"/"+file_name+"/"+imagename+".jpg"
                            image_save_path=save_path+"/"+childdir+"/"+schilddir+"/"+imagename+".jpg"
                            if os.path.exists(image_save_path):
                                    continue
                            else:
                                img=cv2.imread(img_path)#112*410,#1108
                        ...
                        temp_dir1=args.test_img+"/"+childdir+"/"+schilddir
                        for file_name in os.listdir(temp_dir1):
                            img_paths = glob.glob(osp.join(args.test_img+'/'+childdir+'/'+schilddir+'/'+file_name, '*.jpg'))
                            may_mkdirs(save_path+"/"+childdir+"/"+schilddir+"/"+file_name)
                            for img_path in img_paths:
                                imagename,_ =osp.splitext(osp.basename(img_path))
                                # image_save_path=save_path+"/sem"+"/"+file_name+"/"+imagename+".jpg"
                                image_save_path=save_path+"/"+childdir+"/"+schilddir+"/"+file_name+"/"+imagename+".jpg"
                                if os.path.exists(image_save_path):
                                        continue
                                else:
                                    img=cv2.imread(img_path)#112*410,#1108
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        default="/media/data2/lx/cloth-changing/dataset/last",
    )
    parser.add_argument(
